...
The list search_list is a list that contains iterators for objects in storage. This list is ordered. It is logical that there are as many elements in it as in storage, but they are smaller in case the type T is larger than the list < T >:: iterator (this is, almost always).
