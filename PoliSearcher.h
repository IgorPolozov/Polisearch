...
bool operator()(const T &rhs,  const Iter  &lhs)//íà cppreference îøèáêà èëè ó ìàéêðîñîôò?
	{
		return (*predicate)(rhs, *lhs);
	}	
...
bool compire_pred_eq(const T &rhs,  const T& lhs)// ýòî == ïî < èëè >
	{
...
{
if(having_a_free_cell())//ôóíêöèÿ êîððåêòíî ðàáîòàåò òîëüêî åñëè âñå öåïè çàïîëíåíû
//òî åñòü íåò ñâîáîäíîãî ìåñòà, ïîýòîìó òóò îò ãðåõà - ïðîâåðêà è âûõîä åñëè åñòü ñâîáîäíàÿ
//ÿ÷åéêà
	{
		...
		if(bin_search_vec.empty() || search_list.empty())
			{
				...
				if(having_a_free_cell() == false)
					{
						increase_doubling_search_vec();//óäâîèëè ðàçìåð âåêòîðà óìåíüøèâ âäâîå ðàçìåð öåïî÷êè
							add(it_storage);//è âîøëè ñíîâà
								return;
					}
...
if(chain_leader_it->len==1)
	...
	if(chain_leader_it->len==1 && chain_leader_it != bin_search_vec.begin())//ìîæíî ïîèñêàòü àêöåïòîðà ñëåâà
		{
...
void erase_it(const T & val,  ListListedsIter listed_list_it)
    ...
    if(listed_list_it == list_Listeds.end())return;
		Iter storage_it_to_delete=storage.end();
            ...
            storage.erase(storage_it_to_delete);//óäàëåíèå èç õðàíèëèùà           
	}
...
bool is_eq_by_curent_pred=false;
if(fnd_it_start != find_listed->second.search_list.end() /*|| fnd_it_last == fnd_it*/){//íå íàøëà â ãðàíèöàõ
    for( ; fnd_it_start!=fnd_it_last; ++fnd_it_start ){
